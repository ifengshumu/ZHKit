...
#define BOTTOM_SAFE_HEIGHT  ((PORTRAIT ? ScreenHeight : ScreenWidth) >= 800.f ? PORTRAIT ? 34 : 21 : 0)
#define TOP_SAFE_HEIGHT     ((PORTRAIT ? ScreenHeight : ScreenWidth) >= 800.f ? PORTRAIT ? 44 : 0 : 0)
#define HEIGHT              55
#define SPACE               10
#define HEX                 247/255.0
...
- (instancetype)initActionSheetWithTitle:(NSString *)title contents:(NSArray<NSString *> *)contents cancels:(NSArray<NSString *> *)cancels {
    self = [super initWithFrame:CGRectMake(0, TOP_SAFE_HEIGHT, ScreenWidth, ScreenHeight-BOTTOM_SAFE_HEIGHT-TOP_SAFE_HEIGHT)];
    if (self) {
        self.title = title;
...
- (void)orientationDidChange {
    self.frame = CGRectMake(0, TOP_SAFE_HEIGHT, ScreenWidth, ScreenHeight-BOTTOM_SAFE_HEIGHT-TOP_SAFE_HEIGHT);
...
- (void)layoutSubViews {
    ...
    [containerView setCornerRadius:10 corners:UIRectCornerTopLeft | UIRectCornerTopRight];
