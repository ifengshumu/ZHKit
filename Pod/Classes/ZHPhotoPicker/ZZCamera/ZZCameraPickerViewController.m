...
- (void)makeTopViewUI
{
    ...
    //设置闪光灯默认状体为关闭
    UIButton *flashBtn = [[UIButton alloc]initWithFrame:CGRectMake(10, 12, 40, 40)];
    flashBtn.tag = 2015;
    if (self.device.flashMode == 0) {
        [flashBtn setImage:Flash_Off_Pic forState:UIControlStateNormal];
    } else if (self.device.flashMode == 1){
        [flashBtn setImage:Flash_On_Pic forState:UIControlStateNormal];
    } else {
        [flashBtn setImage:Flash_Auto_Pic forState:UIControlStateNormal];
    }
    ...
    UIButton *changeBtn = [[UIButton alloc]initWithFrame:CGRectMake(ZZ_VW - 40, 12, 40, 40)];
    [changeBtn setImage:[UIImage imageNamed:@"zz_camera_turn"] forState:UIControlStateNormal];
...
- (void)makeDownViewUI
{
    ...
    CGFloat h = photoSize + 100;
    _downView = [[UIView alloc] initWithFrame:CGRectMake(0, ZZ_VH - h, ZZ_VW, h)];
    if (_themeColor) {
    ...
    UIButton *cancelBtn = [[UIButton alloc]initWithFrame:CGRectMake(30, h - 60, 50, 30)];
    [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
    [cancelBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [cancelBtn addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
    [_downView addSubview:cancelBtn];
    
    UIButton *doneBtn = [[UIButton alloc]initWithFrame:CGRectMake(ZZ_VW - 80, h - 60, 50, 30)];
    ...
    UIButton *takePhotoBtn = [[UIButton alloc]initWithFrame:CGRectMake(x, h - 70, 50, 50)];
    [takePhotoBtn setImage:[UIImage imageNamed:@"zz_camera"] forState:UIControlStateNormal];
...
- (void)makeCollectionViewUI
{
    ...
    CGFloat photoSize = (ZZ_SCREEN_WIDTH - 60) / 5.0;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIImage *image = [self.cameraArray objectAtIndex:indexPath.row];
    [photoCell loadPhotoDatas:image];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [browserController setModalPresentationStyle:UIModalPresentationFullScreen];
...
- (NSArray *)zzbrowserPickerPhoto:(ZZCameraBrowerViewController *)controller
{
...
- (void)flashOfCamera:(UIButton *)btn
{
    if (btn.tag == 2015) {
        if (self.device.flashMode == 0) {
            ...
            [btn setImage:Flash_Off_Pic forState:UIControlStateNormal];
            
        } else if (self.device.flashMode == 1){
            [self flashLightModel:^{
                [self.device setFlashMode:AVCaptureFlashModeOff];
            }];
            [btn setImage:Flash_On_Pic forState:UIControlStateNormal];
        } else {
            [self flashLightModel:^{
                [self.device setFlashMode:AVCaptureFlashModeAuto];
            }];
            [btn setImage:Flash_Auto_Pic forState:UIControlStateNormal];
        }
    }
}
