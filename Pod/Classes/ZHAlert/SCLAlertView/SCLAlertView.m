...
- (void)viewWillLayoutSubviews
{
    ...
    y += 20.f;
    NSInteger idx = 0;
    CGFloat x = _horizontalButtonEqualWidth ? 0 : 10.f;
    CGFloat space = _horizontalButtonEqualWidth ? 0 : 12.f;
    CGFloat h = 0.f;
    for (SCLButton *btn in _buttons) {
        CGFloat w = _horizontalButtonEqualWidth ? (_windowWidth/_buttons.count) : CGRectGetWidth(btn.frame);
        ...
        if (_horizontalButtons) {
            x += btn.frame.size.width + space;
            if (_horizontalButtonEqualWidth) {
                btn.layer.cornerRadius = 0;
                if (_buttons.count == 1) {
                    [btn setBorderColor:UIColorFromHEX(0xE6E6E6) borderWidth:1 borderSide:UIBorderSideTop];
                } else {
                    if (idx == 0) {
                        [btn setBorderColor:UIColorFromHEX(0xE6E6E6) borderWidth:1 borderSide:UIBorderSideTop|UIBorderSideRight];
                    } else {
                        [btn setBorderColor:UIColorFromHEX(0xE6E6E6) borderWidth:1 borderSide:UIBorderSideTop|UIBorderSideLeft];
                    }
                }
                idx++;
            } else {
...
- (void)buttonTapped:(SCLButton *)btn
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        for (SCLButton *btn in self.buttons)
        {
            btn.actionBlock = nil;
            btn.target = nil;
            btn.selector = nil;
...
- (SCLAlertViewResponder *)showTitle:(UIViewController *)vc image:(UIImage *)image color:(UIColor *)color title:(NSString *)title subTitle:(NSString *)subTitle duration:(NSTimeInterval)duration completeText:(NSString *)completeText style:(SCLAlertViewStyle)style
{
    ...
    if ([subTitle stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]].length > 0) {
        ...
        // Adjust text view size, if necessary
        CGSize sz = CGSizeMake(_windowWidth - 42.0f, CGFLOAT_MAX);
