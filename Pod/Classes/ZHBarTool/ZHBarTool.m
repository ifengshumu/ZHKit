...
@property (nonatomic, assign) ZHScanType scanType;
...
- (void)startScanWithRect:(CGRect)scanRect scanType:(ZHScanType)scanType layerView:(UIView *)layerView {
    //初始化链接对象
    ...
    if ([self.output availableMetadataObjectTypes].count) {
        if (scanType == ZHScanTypeQRCode) {
            self.output.metadataObjectTypes = @[AVMetadataObjectTypeQRCode];
        } else if (scanType == ZHScanTypeBarCode) {
    ...
    [ZHBarTool requestCameraAuthorizedResult:^(BOOL granted) {
        if (granted) {
            ...
            if ([self.delegate respondsToSelector:@selector(authorizedCameraFailed:)]) {
                [self.delegate authorizedCameraFailed:self];
            }
            if (self.authorizedCameraFailed) {
                self.authorizedCameraFailed(self);
            }
        }
    }];
}
...
- (void)captureOutput:(AVCaptureOutput *)output didOutputMetadataObjects:(NSArray<__kindof AVMetadataObject *> *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    if (self.isAvailable) {
        if (metadataObjects.count > 0) {
            ...
            if (self.scanGetObject) {
                self.scanGetObject(self, obj, self.scanType);
            }
            if ([self.delegate respondsToSelector:@selector(barTool:didScanGetObject:sacnType:)]) {
                [self.delegate barTool:self didScanGetObject:obj sacnType:self.scanType];
            }
            self.isAvailable = NO;
