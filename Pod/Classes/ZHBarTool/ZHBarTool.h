...
typedef NS_ENUM(NSUInteger, ZHScanType) {
    ZHScanTypeQRCode = 0,   //二维码
    ZHScanTypeBarCode,      //条形码
    ZHScanTypeAll,          //全部
};
...
///相机无权限
- (void)authorizedCameraFailed:(ZHBarTool *)tool;
///扫描结果
- (void)barTool:(ZHBarTool *)tool didScanGetObject:(NSString *)object sacnType:(ZHScanType)sacnType;
@end
...
/**
 相机无权限Block
 */
@property (nonatomic, copy) void(^authorizedCameraFailed)(ZHBarTool *tool);

/**
 扫描结果Block
 */
@property (nonatomic, copy) void(^scanGetObject)(ZHBarTool *tool, NSString *object, ZHScanType type);
